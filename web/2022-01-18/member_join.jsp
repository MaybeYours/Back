<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 회원정보를 입력받기 위한 JSP 문서 - 자바스크립트를 이용한 입력값 검증 --%>
<%-- => [회원가입]을 클릭한 경우 회원정보 저장 처리페이지(member_join_action.jsp)로 이동 - 입력값(회원정보) 전달 --%>
<%-- => [아이디 중복 검사]를 클릭한 경우 새창에 아이디 중복 검사페이지(id_check.jsp)로 이동 - 아이디 전달 --%>    
<%-- => [우편번호 검색]를 클릭한 경우 새창에 우편번호 검색 출력페이지(post_search.jsp)로 이동 --%>    
<style type="text/css">
fieldset {
	text-align: left;
	margin: 10px auto;
...
<form name="joinForm" id="join" action="<%=request.getContextPath() %>/site/index.jsp?workgroup=member&work=member_join_action" method="post">
<%-- 아이디 중복 검사 실행 여부를 확인하기 위한 입력태그 --%>
<%-- => 입력값 : 0 - 아이디 중복 검사 미실행(아이디 사용 불가능) --%>
<%-- => 입력값 : 1 - 아이디 중복 검사 실행(아이디 사용 가능) --%>
