<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 회원정보를 입력받기 위한 JSP 문서 - 입력페이지 --%>
<!-- => [가입]를 클릭한 경우 처리페이지(join_action.jsp)를 POST 방식으로 요청하여 입력값 전달 -->
<%-- => 자바스트립트를 이용하여 입력값 검증 후 form 태그가 동작되도록 작성 --%>
<%
	/*
	//처리페이지(join_action.jsp)를 비정상적으로 요청한 경우 QueryString으로 전달된 
	//값(에러메세지)을 반환받아 저장
	// => 입력페이지(join_form.jsp)를 정상적으로 요청한 경우 전달값이 없으므로 null 반환
	// => QueryString으로 전달된 값은 리퀘스트 메세지의 캐릭터셋 변경 불필요
	String message=request.getParameter("message");
	//JSP 표현식에 의해 null이 문자열로 출력되는 것을 방지하기 위해 변수에 NullString 저장
	if(message==null) {
		message="";
	}
	*/
	...
	// => 속성값을 Object 타입의 인스턴스로 반환하므로 명시적 객체 형변환 처리 후 사용
	String message=(String)session.getAttribute("message");
	if(message==null) {
		message="";
	} else {
		//다른 JSP 문서에서 에러메세지를 사용하지 못하도록 세션에 저장된 속성값 제거
		//session.removeAttribute(String name) : 바인딩된 세션에 저장된 속성값을 속성명을 사용하여 제거하는 메소드
