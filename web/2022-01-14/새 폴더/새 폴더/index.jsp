...
<%-- => Content 영역은 템플릿 페이지(index.jsp) 요청시 전달되는 값에 따른 다른 JSP 문서의
실행 결과를 동적으로 포함하여 응답 --%>
<%
	//템플릿 페이지 요청시 전달된 값을 반환받아 저장
	String group=request.getParameter("group");//작업 디렉토리
	if(group==null) {
		group="main";
	}
	
	String work=request.getParameter("work");//JSP 파일명
	if(work==null) {
		work="print";
	}
	
	//반환받은 전달값을 이용하여 Content 영역에 포함될 JSP문서의 경로를 생성하여 저장
