...
//1.DataSource 관련 라이브러리와 Mybatis 관련 라이브러리 프로젝트에 빌드 처리 - pom.xml
// => ojdbc, spring-jdbc(spring-tx), mybatis, mybatis-spring
//2.Mybtis Framework의 환경설정파일 작성
//3.DataSource 관련 클래스, SqlSessionFactory 관련 클래스와 SqlSession 관련 클래스를 Spring Bean으로 등록
// => Bean Configuration File(root-context.xml)에서 bean 엘리먼트 사용하여 Spring Bean으로 등록
//4.Table 생성 >> DTO 클래스 작성
...
//2.DataSource 관련 클래스를 Spring Bean으로 등록한 bean 엘리먼트에서 driverClassName 필드값과
//url 필드값을 변경하여 인젝션 처리 - root-context.xml
...
//DAO 클래스를 Spring Annotation으로 Spring Bean으로 등록 
// => Service 클래스에서 DAO 객체를 제공받아 메소드 호출하기 위해 Spring Bean으로 등록
